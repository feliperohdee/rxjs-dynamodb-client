...
module.exports = class Request {
    ...
    addPlaceholderName(name) {
        ...
        if (_.isArray(name)) {
            _.forEach(name, value => {
                return _.set(this.expressionAttributeNames, `#${value}`, value);
            });
        } else {
            _.forEach(name, (value, key) => {
                return _.set(this.expressionAttributeNames, `#${key}`, value);
            });
        }
    ...
    addPlaceholderValue(value) {
        ...
        if (_.isArray(value)) {
            _.forEach(value, value => {
                return _.set(this.expressionAttributeValues, `:${value}`, this.util.anormalizeValue(value));
            });
        } else {
            _.forEach(value, (value, key) => {
                return _.set(this.expressionAttributeValues, `:${key}`, this.util.anormalizeValue(value));
            });
        }
    ...
    get(item) {
        ...
        return this.routeCall('getItem', {
            ...
            .pipe(
                rxop.map(response => {
                    return response.Item ? this.util.normalizeItem(response.Item) : null;
                })
            );
    }
    ...
    update(item, where, insert = false) {
        ...
        return this.routeCall('updateItem', {
            ...
            .pipe(
                rxop.map(response => {
                    return this.util.normalizeItem(response.Attributes);
                })
            );
    }
    ...
    delete(item) {
        ...
        return this.routeCall('deleteItem', {
            ...
            .pipe(
                rxop.map(response => {
                    return response.Attributes ? this.util.normalizeItem(response.Attributes) : null;
                })
            );
    }
    ...
    _batchGetOperation(items) {
        return this.routeCall('batchGetItem', {
            ...
            .pipe(
                rxop.map(response => {
                    return _.get(response, `Responses.${this.tableName}`, null);
                }),
    ...
    batchWrite(toDelete, toInsert) {
        ...
        const transactions = _.map(argsChunks, args => {
            return this.routeCall('batchWriteItem', {
                RequestItems: {
                    [this.tableName]: args
                }
            });
