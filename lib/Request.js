...
module.exports = class Request {
    ...
    queryScan(args = {}, scan) {
        ...
        const queryOperation = (args, index = 0) => {
            return this._queryOperation(args)
                .pipe(
                    rxop.mergeMap(({
                        after,
                        items
                        ...
                        if (_.isFunction(scan)) {
                            items = this.util.normalizeList(items);

                            return scan(items, after, index)
                                .pipe(
                                    rxop.map(({
                                        after,
                                        response = null
    ...
    batchGetScan(items, scan, size = 100) {
        const chunks = _.chunk(items, size);
        const batchGetOperation = (items, index = 0) => {
            return this._batchGetOperation(items)
                .pipe(
                    rxop.mergeMap(response => {
                        const items = this.util.normalizeList(response);
        ...
        return batchGetOperation(chunks[0])
            .pipe(
                rxop.expand(({
                    after
                }, index) => {
                    ...
                    if (after && chunks[index]) {
                        return batchGetOperation(chunks[index], index);
                    }
