...
describe('lib/Crud', () => {
    ...
    after(done => {
        ...
        request.query({
            ...
            .pipe(
                rxop.toArray(),
                rxop.mergeMap(response => request.batchWrite(response))
            )
            .subscribe(null, null, done);
    });
    ...
    describe('fetch', () => {
        ...
        describe('with dataSelector', () => {
            beforeEach(done => {
                const dataSelector = data => data.pipe(
					rxop.map(response => _.pick(response, [
						'id'
					])
				));
        ...
        describe('with customReducer', () => {
            beforeEach(done => {
                const customReducer = data => data.pipe(
					rxop.toArray()
				);
            ...
            it('should query before based on before', done => {
                ...
                query(stats.before)
                    .pipe(
						...
						rxop.mergeMap(response => query(response.stats.before)),
						rxop.tap(response => {
            ...
            it('should query after based on after', done => {
                ...
                query(stats.after)
                    .pipe(
						...
						rxop.mergeMap(response => query(response.stats.after)),
						rxop.tap(response => {
							expect(response.data[0].id).to.equal('id-7');
							expect(response.data[1].id).to.equal('id-8');
							expect(response.stats.count).to.equal(2);
    ...
    describe('updatePrimaryKeys', () => {
        ...
        beforeEach(done => {
            ...
            crud.get({
                ...
                .pipe(
					rxop.tap(response => oldResult = response),
					rxop.mergeMap(() => crud.updatePrimaryKeys({
						namespace,
						id: 'id-0'
					}, {
						id: 'id-00',
    ...
    describe('appendToList', () => {
        ...
        beforeEach(done => {
            crud.insertOrReplace({
                ...
                .pipe(
					rxop.mergeMap(() => crud.appendToList({
						namespace,
						id: 'id-0',
						list: [{
							a: 1
    ...
    describe('prependToList', () => {
        ...
        beforeEach(done => {
            crud.insertOrReplace({
                ...
                .pipe(
					rxop.mergeMap(() => crud.prependToList({
						namespace,
						id: 'id-0',
						list: [{
							a: 1
