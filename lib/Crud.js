...
module.exports = class Crud {
    ...
    fetch(args, hook = false, dataSelector = null, customReducer = null) {
        const {
            after,
            before,
            consistent,
        ...
        let data = request.query.apply(request, hookArgs || [expression]);

        if (_.isFunction(dataSelector)) {
            data = dataSelector(data, request);
        }

        if (_.isFunction(customReducer)) {
            return customReducer(data);
        }
            ...
            rxop.map(data => {
                ...
                if (withCursor) {
                    data = _.map(data, data => {
                        data._cursor = this.toBase64(JSON.stringify(request.getIndexedAttributes(data)));
    ...
    clear(args) {
        ...
        return request.limit(1000)
            .queryScan(expression)
            .pipe(
                rxop.mergeMap(response => this.request.batchWrite(response))
            );
    }
